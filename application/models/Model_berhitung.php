...
class Model_berhitung extends CI_Model {
    ...
    function getQuestion($userId) {
        ...
        $this->saveAnswer($sessionName, $answer);
        return $question;
    }
    ...
    function isAnswerEqual($userId, $userAnswer)
    {
        ...
        $sessionName = $this->getSessionName($userLog['session']);
        $result = [];
        $result['session'] = $this->getSessionName($userLog['session']);
        $result['answer'] = $userLog[$sessionName . '_answer'];
        $result['isEqual'] = ($result['answer'] == $userAnswer);
        $result['levelUpdated'] = false;
        if ($result['isEqual']) {
    ...
    function getSessionName($sessionId) {
        switch ($sessionId) {
            case 1: return "penjumlahan"; break;
            case 2: return "pengurangan"; break;
            case 3: return "perkalian"; break;
        }
    }

    function getSessonId($sessionName) {
        switch ($sessionName) {
            case "penjumlahan": return 1; break;
            case "pengurangan": return 2; break;
            case "perkalian": return 3; break;
        }
    }

    function saveAnswer($session, $ans) {
        $this->db->set($session . "_answer", $ans)
            ->insert('log');
    }

    function getLastSession($userId) {
        $log = $this->select('log.session')
            ->from('log')
            ->where('user_id', $userId)
            ->get()->row_array();
